...
EVENTO Evento(void)
{
	...
	switch(irInBuf.EventType)
	{
		   case KEY_EVENT: /* keyboard input  */
				...
				/* adicionar a tecla ctrl
				if((GetKeyState(VK_LCONTROL) & BIT_SHORT_MAIS_SIGNIFICATIVO) == TECLA_PRESSIONADA)
					Dados_evento.teclado.status_teclas_controle |= LEFT_CTRL_PRESSED;
				if((GetKeyState(VK_RCONTROL) & BIT_SHORT_MAIS_SIGNIFICATIVO) == TECLA_PRESSIONADA)
					Dados_evento.teclado.status_teclas_controle |= RIGHT_CTRL_PRESSED;
				*/
			    break;
